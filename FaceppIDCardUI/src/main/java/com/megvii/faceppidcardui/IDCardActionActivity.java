...
public class IDCardActionActivity extends Activity implements View.OnClickListener {
	...
	private int[] imageItemImages_gray = { R.drawable.textrec_gray, R.drawable.shader_gray, R.drawable.debug_gray,
			R.drawable.verticalscreen };
	private int[] imageItemImages_blue = { R.drawable.textrec_blue, R.drawable.shader_blue, R.drawable.debug_blue,
			R.drawable.horizontalscreen };
	...
	private void init() {
		...
		RelativeLayout rel0 = (RelativeLayout) findViewById(R.id.idcardAction_imageitem_0);
		RelativeLayout rel1 = (RelativeLayout) findViewById(R.id.idcardAction_imageitem_1);
		RelativeLayout rel2 = (RelativeLayout) findViewById(R.id.idcardAction_imageitem_2);
		RelativeLayout rel3 = (RelativeLayout) findViewById(R.id.idcardAction_imageitem_3);
		imageItem_Rels = new RelativeLayout[] { rel0, rel1, rel2, rel3 };
		RelativeLayout textRel0 = (RelativeLayout) findViewById(R.id.idcardAction_edititem_0);
		RelativeLayout textRel1 = (RelativeLayout) findViewById(R.id.idcardAction_edititem_1);
		RelativeLayout textRel2 = (RelativeLayout) findViewById(R.id.idcardAction_edititem_2);
	...
	public void onClick(View v) {
		...
		if(ID == R.id.activity_rootRel){
		...
		} else if(ID == R.id.title_layout_returnRel){
			finish();
		} else if(ID == R.id.idcardAction_edititem_0){
			mDialogUtil.showEditText(editItemTexts[0], 2);
		} else if(ID == R.id.idcardAction_edititem_1){
			mDialogUtil.showEditText(editItemTexts[1], 3);
		} else if(ID == R.id.idcardAction_edititem_2){
			mDialogUtil.showEditText(editItemTexts[2], 4);
		} else if(ID == R.id.idcardAction_imageitem_0){
			isTextDetect = !isTextDetect;
			onclickImageItem(0, isTextDetect);
		} else if(ID == R.id.idcardAction_imageitem_1){
			isClearShadow = !isClearShadow;
			onclickImageItem(1, isClearShadow);
		} else if(ID == R.id.idcardAction_imageitem_2){
			isDebug = !isDebug;
			onclickImageItem(2, isDebug);
		} else if(ID == R.id.idcardAction_imageitem_3){
			isHorizontal = !isHorizontal;
			onclickImageItem(3, isHorizontal);
		} else if(ID == R.id.idcardAction_enterBtn){
			Intent intent = new Intent(this, IDCardScanActivity.class);
			Toast.makeText(IDCardActionActivity.this,"-----",Toast.LENGTH_LONG).show();
			...
			Log.i("tag"," bound="+editValue[0] +"idcard="+ editValue[1]+" clear="+editValue[2]);
